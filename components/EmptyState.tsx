  ...
  return (
    <div className="text-center mt-[10vh] text-slate-500 dark:text-slate-400 p-4 animate-fade-in">
        ...
        <div className="w-full max-w-md mx-auto my-6">
          <div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden mb-2">
            <div 
              ...
              {/* FIX: Refactored progress bar width calculation to use `isError` and `Math.round` for clarity and to potentially resolve type inference issues. */}
      ...
      {status !== 'loading' && (
        <button 
          onClick={onLoad} 
          {/* FIX: The button is only shown when `status` is not 'loading', so it should always be enabled here. */}
          disabled={false}
      ...
      <div className="mt-8 text-xs text-slate-400">
        <p><i className="fa-solid fa-lightbulb"></i> Conseil : Utilise Chrome/Edge avec WebGPU activé</p>
      </div>
    </div>
  );
};
