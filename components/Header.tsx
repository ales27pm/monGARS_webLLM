    ...
    <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-3 flex justify-between items-center h-[60px] flex-shrink-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-baseline">
          <span className="font-bold text-xl text-primary-DEFAULT">
            Mon Gars
          </span>
      ...
      <div className="flex gap-1">
        ...
        <IconButton onClick={onVoiceMode} title="Mode conversation vocale">
...
const IconButton: React.FC<{
  ...
  <button
    onClick={onClick}
    title={title}
    className="w-9 h-9 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-primary-DEFAULT rounded-md transition-colors"
  >
    {children}
  </button>
);
